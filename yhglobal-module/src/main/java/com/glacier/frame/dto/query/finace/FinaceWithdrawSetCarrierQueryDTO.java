...
/**
 * @ClassName: FinaceWithdrawSetCarrierQueryDTO 
 * @Description: TODO(提现类型查询DTO) 
 * @author wuting
 * @email 920339213@QQ.com
 * @date 2014-10-15
 */
public class FinaceWithdrawSetCarrierQueryDTO extends FinaceWithdrawSetCarrier{
   ...
   public String getLoginStartCount() {
	...
	public void setQueryCondition(Criteria queryCriteria){
   	 if(null != this.getWithdrawName() && StringUtils.isNotBlank(this.getWithdrawName())){//充值名称Like查询
            queryCriteria.andWithdrawNameLike("%" + this.getWithdrawName() + "%");
        }  
     if(null != createStartTime && null != createEndTime){//创建时间段查询
           queryCriteria.andCraeteTimeBetween(createStartTime, createEndTime); 
     }else{
          if(null != createStartTime){
              queryCriteria.andCraeteTimeGreaterThanOrEqualTo(createStartTime);
          }
          if(null != createEndTime){
              queryCriteria.andCraeteTimeLessThanOrEqualTo(createEndTime);
          } 
       }
    }
