...
/**
 * @ClassName: FinaceRechargeCarrierQueryDTO 
 * @Description: TODO(货物查询DTO) 
 * @author wuting
 * @email 920339213@QQ.com
 * @date 2014-10-15
 */
public class FinaceRechargeCarrierQueryDTO extends FinaceRechargeCarrier{
   ...
   public String getLoginStartCount() {
	...
	public void setQueryCondition(Criteria queryCriteria){
   	 if(null != this.getRechargeCode() && StringUtils.isNotBlank(this.getRechargeCode())){//充值名称Like查询
            queryCriteria.andRechargeCodeLike("%" + this.getRechargeCode() + "%");
        }  
   	 if(null != this.getRechargeSetDisplay() && StringUtils.isNotBlank(this.getRechargeSetDisplay())){//充值名称Like查询
        queryCriteria.andRechargeSetDisplayLike("%" + this.getRechargeSetDisplay() + "%");
    	}
