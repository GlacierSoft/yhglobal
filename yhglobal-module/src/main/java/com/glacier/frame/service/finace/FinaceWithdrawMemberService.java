...
public class FinaceWithdrawMemberService {
	...
	public Object addWithdraw(FinaceWithdrawMember finaceWithdrawMember,String tradersPassword, String bankCardId, int mobile_code,int mobile_true) {
		...
		finaceWithdrawSetMemberExample.createCriteria().andFeeWayEqualTo("proportion").andAuditStateEqualTo("pass");
		List<FinaceWithdrawSetMember> financeWithdrawSets = finaceWithdrawSetMemberMapper.selectByExample(finaceWithdrawSetMemberExample);
		if (financeWithdrawSets != null && financeWithdrawSets.size() > 0) {
			FinaceWithdrawSetMember financeWithdrawSet = financeWithdrawSets.get(0);
			finaceWithdrawMember.setChargeMoney(finaceWithdrawMember.getWithdrawMoney().multiply(financeWithdrawSet.getWithdrawRate()));
			finaceWithdrawMember.setWithdrawReallyMoney(finaceWithdrawMember.getWithdrawMoney().add(finaceWithdrawMember.getChargeMoney()));
		} else {
			returnResult.setMsg("后台提现方式未确定，请联系管理员!");
			return returnResult;
		}
		if (finaceWithdrawMember.getWithdrawReallyMoney().compareTo(financeMember.getMrechageRemain()) == 1) {
			returnResult.setMsg("手续费不足,请及时充值或者减少提现现金!");
			return returnResult;
		}
		// 构建提现对象
		...
		finaceWithdrawMember.setWithdrawSetId(financeWithdrawSets.get(0).getWithdrawSetId());
	...
	public Object updateFinaceMember(String id) {
		...
		if (id != null) {
			...
			FinanceMember financeMember = financeMemberMapper.selectByExample(financeMemberExample).get(0);
			BigDecimal memberRechargeRemain = financeMember.getMrechageRemain();
			financeMember.setMrechageRemain(memberRechargeRemain.subtract(finaceWithdrawMember.getWithdrawReallyMoney()));
