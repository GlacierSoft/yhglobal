...
<script type="text/javascript">
	...
	glacier.finance_mgr.financePlatform_mgr.financePlatform.financePlatformDataGrid = $('#financePlatformDataGrid').datagrid({
		...
		onDblClickRow:function(rowIndex, rowData){
			$.easyui.showDialog({
				...
				href : ctx + '/do/financePlatform/intoDetail.htm?platformId='+rowData.platformId,//从controller请求jsp页面进行渲染
				width : 600,
				height : 375,
				resizable: false,
      	...
      	$.messager.confirm('请确认', '是否要设置为默认账号?', function(r){
			if (r){
				$.ajax({
					   ...
					   data: {platformId:row.platformId},
					   dataType:'json',
					   success: function(r){
	...
	glacier.finance_mgr.financePlatform_mgr.financePlatform.delFinancePlatform = function(){
		...
		for(var i=0;i<rows.length;i++){
			financePlatformIds.push(rows[i].platformId);
