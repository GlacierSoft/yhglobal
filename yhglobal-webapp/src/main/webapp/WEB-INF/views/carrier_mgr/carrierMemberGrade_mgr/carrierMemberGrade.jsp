...
<script type="text/javascript">
	...
	glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.memberGradeDataGrid = $('#memberGradeDataGrid').datagrid({
                        ...
                        $.easyui.showDialog({
								...
								href : ctx+ '/do/carrierMemberGrade/intoDetail.htm?gradeId='+ rowData.gradeId,//从controller请求jsp页面进行渲染
								width : 830,
								height : 370,
								resizable : false,
	...
	glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.addMemberGrade = function(){
		glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.memberGradeDialog('【承运商等级】- 增加承运商等级',false,'/do/carrierMemberGrade/add.json');
	};
	
	//编辑会员信用等级
	glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.editMemberGrade = function(){
		glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.memberGradeDialog('【承运商等级】- 编辑承运商等级',true,'/do/carrierMemberGrade/edit.json');
	};
	...
	*/
	glacier.memberGrade_mgr.memberGrade_mgr.memberGrade.memberGradeDialog = function(title,editModel,url){
		$.easyui.showDialog({
			href : ctx + '/do/carrierMemberGrade/intoForm.htm',//从controller请求jsp页面进行渲染
			width : 410,
			height : 350,
			resizable: false,
			enableSaveButton : false,
		...
		if (memberGradeIds.length > 0) {
			$.messager.confirm('请确认','是否要删除该记录',function(r){
                   if (r){
                   	 $.ajax({ 
                   		type: "POST",
                   	    url : ctx+ '/do/carrierMemberGrade/del.json',
						data : {
...
<div class="easyui-layout" data-options="fit:true">
	<div id="memberGradeGridPanel" data-options="region:'center',border:true">
		<table id="memberGradeDataGrid">
			<glacierui:toolbar panelEnName="CarrierMemberGradList"
				toolbarId="memberGradeDataGrid_toolbar" menuEnName="carrierMemberGrade" />
