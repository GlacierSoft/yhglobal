...
public class CaptchaFormAuthenticationFilter extends FormAuthenticationFilter {
    ...
    protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) { 
        ...
        if(email.matches(reg)) { //匹配的是邮箱，那么进行邮箱登陆验证  
            ShipperMember mem=shipperMemberService.retrieveEmail(email);//通过该邮箱，看是否存在
            if(mem!=null){//说明数据库存在 
                //是否禁用了该会员
            ...
            ShipperMember shipperMember=shipperMemberService.retrieveName(email);
            if(shipperMember!=null){
                //是否禁用了该会员
                if(shipperMember.getStatus().equals("disable")){
                    username="NO";//不让它登陆
                }else{ 
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
           ...
           Subject subject = getSubject(request, response);
            ...
            ShipperMember shipperMember = (ShipperMember) subject.getPrincipal(); 
       /*     session.setAttribute("currentShipperMember", shipperMember);
            session.setAttribute("currentShipperMemberWork", shipperMemberService.getShipperMemberWork(shipperMember.getShipperMemberId()));
            int messageNoticCount = loginTotalMessageNotic(shipperMember.getShipperMemberId()); 
            session.setAttribute("messageNoticCount", messageNoticCount); 
            shipperMemberService.updateIntegra(shipperMember.getShipperMemberId());*/
 ...
 /*   *//**
     ...
     *
     *//*
    public int loginTotalMessageNotic(String shipperMemberId){
        //设置查询DTO收信人的id
         ShipperMemberMessageNoticeQueryDTO shipperMemberMessageNoticeQueryDTO = new ShipperMemberMessageNoticeQueryDTO();
         shipperMemberMessageNoticeQueryDTO.setAddressee(shipperMemberId);
         shipperMemberMessageNoticeQueryDTO.setLetterstatus("unread");
         JqPager pager = new JqPager();
        //获取信息通知列表
         JqGridReturn returnResult = (JqGridReturn) shipperMemberMessageNoticeService.listAsGridWebsite(shipperMemberMessageNoticeQueryDTO, pager,1);
