...
<script type="text/javascript"> 
	...
	//创建地图函数：
	function createMap() {
	//	var sContent ="${requestScope.goodsArea.address}";
		var map = new BMap.Map("dituContent");//在百度地图容器中创建一个地图
		var point = new BMap.Point(${requestScope.goodsArea.longitude},${requestScope.goodsArea.latitude});//定义一个中心点坐标
		map.centerAndZoom(point, 18);//设定地图的中心点和坐标并将地图显示在地图容器中 
		map.openInfoWindow(createInfoWindow(0),point); 
	...
	//标注点数组
	var markerArr = [ {
		title : "${requestScope.goodsArea.deliverName}-物流区",
		content :"<span style='font-size:12px;'>联系电话:${requestScope.goodsArea.telephone}</span>"
		+"<br><span style='font-size:12px;'>业务:${requestScope.goodsArea.serviceContent}</span>"
		+"<br><span style='font-size:12px;'>地址:${requestScope.goodsArea.address}</span>",
		point : "${requestScope.goodsArea.longitude}|${requestScope.goodsArea.latitude}",
		isOpen : 0,
		icon : {
			w : 21,
			h : 21,
